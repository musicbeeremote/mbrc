...
class ArtistRepositoryTest : KoinTest {
  ...
  private val dao: ArtistDao by inject()
  ...
  fun getRemoteShouldFetchAndStoreNewArtists() {
    runTest(testDispatcher) {
      ...
      every { libraryApi.getArtists(any()) } returns flowOf(remoteArtists)
  ...
  fun getRemoteShouldHandleProgressCallback() {
    runTest(testDispatcher) {
      ...
      every { libraryApi.getArtists(progress) } returns flowOf(remoteArtists)

      repository.getRemote(progress)

      @Suppress("IgnoredReturnValue")
      verify { libraryApi.getArtists(progress) }
    }
  }

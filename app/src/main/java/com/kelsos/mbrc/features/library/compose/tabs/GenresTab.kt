...
import com.kelsos.mbrc.features.library.genres.GenreUiMessage
...
fun GenresTab(
  ...
  val queueResults = remember {
    viewModel.events.map { event ->
      when (event) {
        is GenreUiMessage.QueueSuccess -> QueueResult.Success(event.tracksCount)
        is GenreUiMessage.QueueFailed -> QueueResult.Failed
        is GenreUiMessage.NetworkUnavailable -> QueueResult.NetworkUnavailable
        else -> null
      }

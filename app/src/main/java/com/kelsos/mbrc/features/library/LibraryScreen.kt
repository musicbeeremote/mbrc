...
fun LibraryScreen(openDrawer: () -> Unit) {
  ...
  LibraryScreen(openDrawer = openDrawer, sync = { vm.refresh() })
}

@OptIn(ExperimentalPagerApi::class)
@Composable
fun LibraryScreen(openDrawer: () -> Unit, sync: () -> Unit) = Surface {
  val tabs = listOf(
  ...
  Column(modifier = Modifier.fillMaxSize()) {
    ...
    HorizontalPager(
      ...
      when (pagerState.currentPage) {
        0 -> GenresScreen(sync = sync)
        1 -> ArtistsScreen(sync = sync)
        2 -> AlbumsScreen(sync = sync)
        3 -> TracksScreen(sync = sync)
      }
    }
  }
}

...
@Composable
fun LibrarySyncIndicator(syncProgress: LibrarySyncProgress) =
  Surface(
  ...
  Column(modifier = Modifier.fillMaxSize()) {
    ...
    HorizontalPager(
      ...
      when (page) {
        ...
        Pages.ARTISTS ->
          ArtistsScreen(sync = sync) { queue, id ->
            action(id, Meta.Artist, queue)
          }
        Pages.ALBUMS ->
          AlbumsScreen(sync = sync) { queue, id ->
            action(id, Meta.Album, queue)
          }
        Pages.TRACKS ->
          TracksScreen(sync = sync) { queue, id ->

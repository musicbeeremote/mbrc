...
class QueueUseCaseImpl(
  ...
  override suspend fun queue(id: Int, @Type meta: Int, @Action action: String) = withContext(dispatchers.disk) {
  ...
  private suspend fun tracks(id: Int, @Action action: String): Pair<List<String>, String?> = withContext(dispatchers.database) {
    val track = trackRepository.getById(id)
    if (track != null) {
      when (action) {
        Queue.ADD_ALBUM -> Pair(trackRepository.getAlbumTrackPaths(track.album, track.albumArtist), track.src)

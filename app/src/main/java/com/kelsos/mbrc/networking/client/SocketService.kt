...
class SocketService(
...
) : SocketActivityChecker.PingTimeoutListener {
  ...
  init {
    resetState()
    bus.register(this, DefaultSettingsChangedEvent::class.java) { socketManager(SocketAction.RESET) }
  }

  private fun startSocket() {
    context.cancelChildren()
    scope.launch {
      Timber.Forest.d("Trying to connect. Try %d of %d", (numOfRetries + 1), MAX_RETRIES)
      try {
        delay(3000)
        val settings = connectionRepository.getDefault()
        if (settings == null) {
          socketManager(SocketAction.STOP)
          bus.post(MessageEvent(ApplicationEvents.TERMINATE_SERVICE))
          return@launch
        }

        Timber.Forest.v("Connecting to mbrc://${settings.address}:${settings.port}")
        executor.execute(SocketConnection(settings))
        numOfRetries++
      } catch (e: Exception) {
        Timber.Forest.v(e, "Connection failed")
      }
    }
  }

  fun socketManager(
    @SocketAction.Action action: Int,
  ) {
    Timber.Forest.v("Received action ${SocketAction.name(action)}")
    when (action) {
      SocketAction.RESET -> {
        shouldStop = false
        resetState()
   ...
   * @return Boolean
   */
  private fun isConnected(): Boolean = socket?.isConnected ?: false

  private fun cleanupSocket() {
    Timber.Forest.v("Cleaning up socket")
    if (!isConnected()) {
      return
    }
    try {
  ...
  fun sendData(message: SocketMessage) {
    try {
      ...
      Timber.Forest.d(ignored, "Send failed")
    }
  }

  override fun onTimeout() {
    Timber.Forest.v("Timeout received. Resetting socket")
  ...
  private inner class SocketConnection(
    ...
    override fun run() {
      Timber.Forest.v("Socket Start")
      bus.post(MessageEvent(ApplicationEvents.SOCKET_HANDSHAKE_UPDATE, false))
      ...
      try {
        ...
        bus.post(MessageEvent(ApplicationEvents.SOCKET_STATUS_CHANGED, socketStatus))
        ...
        while (socket!!.isConnected) {
          try {
            ...
            if (incoming.isNotEmpty()) {
              bus.post(MessageEvent(ApplicationEvents.SOCKET_DATA_AVAILABLE, incoming))
            }
          } catch (e: IOException) {
            input.close()
            socket?.close()
        ...
        Timber.Forest.v(ignored, "IO")
      } catch (npe: NullPointerException) {
        Timber.Forest.e(npe, "NPE")
      } finally {
        output?.close()
        ...
        Timber.Forest.d("Socket closed")
        bus.post(MessageEvent(ApplicationEvents.SOCKET_STATUS_CHANGED, false))

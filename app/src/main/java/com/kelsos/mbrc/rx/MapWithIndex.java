...
public final class MapWithIndex<T> implements Transformer<T, MapWithIndex.Indexed<T>> {
  ...
  @SuppressWarnings("unchecked") public static <T> MapWithIndex<T> instance() {
    return (MapWithIndex<T>) Holder.INSTANCE;
  }

  @Override public Observable<Indexed<T>> call(Observable<T> source) {

    return source.zipWith(NaturalNumbers.instance(), new Func2<T, Long, Indexed<T>>() {

      @Override public Indexed<T> call(T t, Long n) {

...
fun <T : Any> pagingDataFlow(vararg elements: T) =
  flow {
...
fun <T : Any> ScreenContent(
  ...
  LazyColumn(
    ...
    items(count = items.itemCount, key = items.itemKey { key(it) }) { index ->
      if (items[index] != null) {
...
fun <T : Any> SwipeRefreshScreen(
  ...
  val pullRefreshState = rememberPullRefreshState(content.isRefreshing, { content.onRefresh() })
...
@Composable
fun PopupMenu(menuContent: @Composable (ColumnScope.() -> Unit)) =
  Column {

...
@ModelView(database = RemoteDatabase::class,
  ...
  companion object {
    @JvmField
    @ModelViewQuery val QUERY: Query = SQLite.select(PlaylistTrackDao_Table.id.withTable(),
        ...
        PlaylistTrackInfoDao_Table.title.withTable()).from(PlaylistTrackDao::class.java).join(PlaylistTrackInfoDao::class.java,
        Join.JoinType.INNER).on(PlaylistTrackInfoDao_Table.id.withTable().`is`(PlaylistTrackDao_Table.track_info_id)).orderBy(
        OrderBy.fromProperty(PlaylistTrackDao_Table.playlist_id).ascending()).orderBy(OrderBy.fromProperty(
        PlaylistTrackDao_Table.position).ascending())
  }
}

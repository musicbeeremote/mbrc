...
class TrackRepositoryImpl : TrackRepository {
    ...
    override fun getTrackInfo(reload: Boolean): Observable<TrackInfo> {
        ...
        return if (reload)
            ...
            Observable.concat(Observable.just(cache.trackinfo), infoObservable).filter {
                trackInfo ->
                trackInfo != null
            }.first()
    }
    ...
    override fun getTrackLyrics(reload: Boolean): Observable<List<String>> {
        ...
        val remote = trackService.getTrackLyrics()
                ...
                .flatMap<List<String>>(Func1 {
                    ...
                    val lyricsList = if (TextUtils.isEmpty(trackLyrics))
                        emptyList<String>()
                    else
                        ArrayList(Arrays.asList<String>(*trackLyrics.split("\r\n".toRegex())
        ...
        return if (reload)
            ...
            Observable.concat(Observable.just(cache.lyrics), remote).filter { strings -> strings != null }.first()
    }
    ...
    override fun getTrackCover(reload: Boolean): Observable<Bitmap?> {
        ...
        return if (reload)
            ...
            Observable.concat(Observable.just(cache.cover), remote)
                    .filter {
                        bitmap ->
                        bitmap != null
                    }.first()

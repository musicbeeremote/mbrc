...
constructor(context: Activity) : SectionedRecyclerViewAdapter<SearchResultAdapter.SearchViewHolder>() {
  ...
  override fun onBindViewHolder(holder: SearchViewHolder,
    ...
    when (section) {
      SECTION_ALBUM -> {
        ...
        holder.lineOne.text = album.name
      ...
      SECTION_ARTIST -> {
        ...
        holder.lineOne.text = artist.name
      ...
      SECTION_GENRE -> {
        ...
        holder.lineOne.text = genre.name
  ...
  private fun onContextClick(holder: SearchViewHolder, track: Track) {
    showPopup(R.menu.popup_track, holder.uiItemContextIndicator!!, {
      onSearchItemSelectedListener?.trackSelected(it, track)
      true
    })
  }

  private fun onContextClick(holder: SearchViewHolder, genre: Genre) {
    showPopup(R.menu.popup_genre, holder.uiItemContextIndicator!!, {
      onSearchItemSelectedListener?.genreSelected(it, genre)
      true
    })
  }

  private fun onContextClick(holder: SearchViewHolder, album: Album) {
    showPopup(R.menu.popup_album, holder.uiItemContextIndicator!!, {
      onSearchItemSelectedListener?.albumSelected(it, album)
      true
    })
  }

  private fun onContextClick(holder: SearchViewHolder, artist: Artist) {
    showPopup(R.menu.popup_artist, holder.uiItemContextIndicator!!, {
      onSearchItemSelectedListener?.artistSelected(it, artist)
      true
    })
  }

...
public class DrawerFragment extends RoboListFragment
  ...
  @InjectView(R.id.menu_exit) private TextView exitButton;
  ...
  @Override public void onStart() {
    super.onStart();
    menuConnector.setOnClickListener(
        v -> Events.messages.onNext(new Message(EventType.START_CONNECTION)));
    ...
    versionIndicator.setText(
        String.format(getString(R.string.ui_drawer_menu_version), BuildConfig.VERSION_NAME));
  }

  private void createObservable() {
    drawerSelectionObservable = Observable.create(
        subscriber -> getListView().setOnItemClickListener((parent, view, position, id) -> {
  ...
  public void handleConnectionStatusChange(final ConnectionStatusChange change) {
    ...
    switch (change.getStatus()) {
      case CONNECTION_OFF:
        menuConnector.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_connectivy_off, 0, 0,
            0);
        menuConnector.setText(R.string.drawer_connection_status_off);
        break;
      case CONNECTION_ACTIVE:
        menuConnector.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_connectivity_active, 0,
            0, 0);
